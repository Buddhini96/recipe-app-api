...
class PublicUserApiTests(TestCase):
    ...
    def test_user_with_email_exists_error(self):
        """Test error returns an error when user with email exists"""
        payload = {
            'email': 'test@example.com',
            'password': 'testpass123',
            'name': 'Test Name',
    ...
    def test_password_too_short_error(self):
        """Test an error is returns if password less than 5 chars."""
        payload = {
            'email': 'testpasswordcheck@example.com',
            'password': 'pw',
            'name': 'Test Name',
        }
        res = self.client.post(CREATE_USER_URL, payload)
    ...
    def test_create_token_bad_credentials(self):
        """Test returns error for bad credentials."""
        user_details = {
            'name': 'Test Name',
            'email': 'test@example.com',
            'password': 'test-user-password123', 
...
class PrivateUserApiTests(TestCase):
    ...
    def test_update_user_profile(self):
        ...
        payload = {"name": "Update name", "password": "updated_password"}
